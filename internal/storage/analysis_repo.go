...
func SaveAnalysis(a *core.Analysis) error {
	...
	query := `INSERT INTO analyses (id, case_id, findings, risks, connections, next_steps, confidence, analyzed_at) 
	          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
	_, err := DB.Exec(query, a.ID, a.CaseID, string(findingsJSON), string(risksJSON), string(connectionsJSON), string(nextStepsJSON), a.Confidence, a.AnalyzedAt)
	if err != nil {
...
func GetLatestAnalysis(caseID string) (*core.Analysis, error) {
	...
	query := `SELECT id, case_id, findings, risks, connections, next_steps, confidence, analyzed_at 
	...
	err := row.Scan(&a.ID, &a.CaseID, &findingsStr, &risksStr, &connStr, &nextStepsStr, &a.Confidence, &a.AnalyzedAt)
